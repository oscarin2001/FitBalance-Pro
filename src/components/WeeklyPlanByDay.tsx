                      ...
                      <div key={i} className="grid items-start gap-2 sm:grid-cols-[9rem_1fr]">
                        ...
                        <div className="min-w-0 leading-snug break-words">
                          <div className="flex flex-wrap items-baseline gap-1">
                            ...
                            {/* Mostrar ingredientes si existen (forma usada por fallback/IA en producción) */}
                            {(!Array.isArray(m.itemsText) || m.itemsText.length === 0) && Array.isArray((m as any).ingredientes) && (m as any).ingredientes.length > 0 && (
                              <ul className="mt-1 list-disc pl-5 space-y-0.5 text-muted-foreground break-words">
                                {(m as any).ingredientes.map((ing: any, j: number) => (
                                  <li key={j}>{ing?.nombre ?? ing?.name ?? JSON.stringify(ing)}{ing && (ing?.gramos || ing?.g) ? ` (${Number(ing?.gramos || ing?.g)} g)` : ''}</li>
                                ))}
                              </ul>
                            )}
                          {bevList.length > 0 && (
