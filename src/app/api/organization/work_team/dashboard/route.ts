...
export async function GET() {
  try {
    ...
    const [
      ...
      prisma.clientSubscription.count({ where: { status: "ACTIVE" } }),
      prisma.clientSubscription.count({ where: { status: "CANCELED" } }),
    ]);
    ...
    // Return a similar shape to global KPIs; frontends can map fields as needed.
    return NextResponse.json({
      ...
      retentionRate,
      churnRate,
      // extras for work_team consumers
      totalTeams: 0,
      totalMembers: totalClients,
    });
  } catch (error) {
    console.error("Error cargando KPIs work_team", error);
    return NextResponse.json(
      { error: "No se pudieron cargar las métricas de work_team" },
      { status: 500 }
    );
  }
}
