...
// Treat this route as dynamic and run in the Node.js runtime to avoid
// Next.js sync-dynamic API errors from middleware that inspects headers/cookies.
...
const authOptions: any = {
  ...
  // Force cookie name and options so development (http) receives a cookie
  cookies: {
    sessionToken: {
      name:
  ...
  callbacks: {
    async signIn({ account, profile }: any) {
      try {
        ...
        if (emailVerified === false || !email) {
          console.warn(
            "[NEXTAUTH][signIn][google] blocked: emailVerified=",
            emailVerified,
            "email=",
            email
          );
        ...
        const { created, usuario } = await ensureUser(
          email,
          (profile as any)?.name
        );
        ...
        console.log(
          "[NEXTAUTH][signIn][google] proceed user created?",
          created
        );
        return true;
      } catch (e) {
    ...
    async session({ session, token }: any) {
      if (token) {
        ...
        (session as any).onboarding_completed =
          token.onboarding_completed ?? null;
...
// (dynamic/runtime already exported above)
